...
#include <stdlib.h>
#include "EPD_IO.h"
// #include <stdint.h>  // Ensure uint8_t is available
...
typedef unsigned long  UDOUBLE; // 32-bit unsigned

// Display resolution
#define EPD_WIDTH 1200
#define EPD_HEIGHT 1600
// EL133UF1，有两个CS(Frame)每个字节包含两个像素，每个像素4bit
#define EPD_IMAGE_SIZE  (EPD_HEIGHT * EPD_WIDTH)  //1920000 - 为了存储方便，一个byte一个像素
...
public:
  ...
  void EL133UF1_Clear(UBYTE color, UBYTE* frame_buffer_m, UBYTE* frame_buffer_s);
  void EL133UF1_DisplayPart(const UBYTE *Image, UWORD xstart, UWORD ystart, 
                            UWORD image_width, UWORD image_height, 
                            UBYTE *frame_buffer_m, UBYTE *frame_buffer_s);
